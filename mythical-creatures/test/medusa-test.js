...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    // create medusa
    var medusa = new Medusa('Jack');
    // create first victim
    var victim1 = new Person('Dead guy');
    // create second victim
    var victim2 = new Person('Dead girl');
    // create third victim
    var victim3 = new Person('Peet');
    // create fourth victim
    var victim4 = new Person('Jimmy');

    // medusa stares at first victim
    medusa.stare(victim1);
    // assert that the first victim is stoned
    assert.equal(medusa.statues[0].name, 'Dead guy');
    ...
    assert.equal(medusa.statues[0].name, 'Jimmy')

    // assert that the first victim is not stoned
    assert.equal(medusa.statues[2].name, 'Dead girl')
